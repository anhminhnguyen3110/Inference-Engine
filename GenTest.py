...
from common import construct_expression_tree, fail, infix_to_postfix, postfix_to_infix, prefix_to_infix
from constants import DEPTH, GENERAL_METHODS, HORN_METHODS, NUM_SYMBOLS_HIGH, NUM_SYMBOLS_LOW, NUMBER_OF_GENERAL_CLAUSES, NUMBER_OF_HORN_CLAUSES, NUMBER_OF_SYMBOLS_FIX, PROGRAM_PATH_1, PROGRAM_PATH_2, TEST_FILE_NAME, TEST_FOLDER_NAME
    ...
    if(len(sys.argv) <= 1):
        ...
        if(test == "cnf"):
            for _ in range(100000):
                input = generate_random_tests(NUMBER_OF_SYMBOLS_FIX, NUMBER_OF_GENERAL_CLAUSES, DEPTH).split("; ")[1]
                testCNF(input)
        elif(test == "horn"):
            run_horn_tests(1000)
        elif(test == "general"):
